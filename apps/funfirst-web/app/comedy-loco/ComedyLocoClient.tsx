...
export default function ComedyLocoClient() {
  useEffect(() => {
    ...
    const selectors = [
      "#what .order-2",
      "#what .cl-media-frame",
      "#how .mx-auto.max-w-2xl",
      ".cl-xp-item",
      "#how .cl-media-frame--wide",
      "#teams .mx-auto.max-w-2xl",
      ".cl-team-card",
      "#teams .cl-media-frame",
      ".cl-waitlist-panel",
    ];
    const revealTargets = document.querySelectorAll(selectors.join(","));
    revealTargets.forEach((el) => el.classList.add("cl-fade-up"));
    ...
    const onSubmit = async (e: Event) => {
      ...
      const label = submitBtn?.querySelector(".cl-btn-label");
      if (label) label.textContent = "Locking in…";

      await new Promise((resolve) => setTimeout(resolve, 550));

      try {
        const existing = JSON.parse(localStorage.getItem("comedyLocoWaitlist") || "[]");
        existing.push({ name, email, joinedAt: new Date().toISOString() });
        localStorage.setItem("comedyLocoWaitlist", JSON.stringify(existing));
      } catch {
