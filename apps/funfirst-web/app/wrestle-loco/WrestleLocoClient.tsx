...
export default function WrestleLocoClient() {
  useEffect(() => {
    ...
    const onSubmit = async (e: Event) => {
      ...
      try {
        const existing = JSON.parse(localStorage.getItem("wrestleLocoWaitlist") || "[]");
        existing.push({ name, email, joinedAt: new Date().toISOString() });
        localStorage.setItem("wrestleLocoWaitlist", JSON.stringify(existing));
      } catch {
