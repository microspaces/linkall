 ...
 *
 *   pnpm --filter @linkall/backend exec convex run wrestlelocoLuxor:seed --env-file .env.funfirst
 *   pnpm --filter @linkall/backend exec convex run seed:wrestleLocoLuxor --env-file .env.funfirst
 *
...
export const SHOW_SOURCE_KEY = "wl-luxor:show";
export const SHOW_TITLE = "Wrestle Loco — Luxor Night 1";
export const SHOW_DESCRIPTION =
  "Luxor Las Vegas — Faces vs Heels. Five matches, award ceremony, outro.";
export const PERFORMANCE_TITLE = "Wrestle Loco — Luxor Night 1";

const FX = "/wrestle-loco/effects";

type MediaKind = "image" | "video";
type Asset = { kind: MediaKind; content: string };

const ASSETS = {
  sweep: { kind: "video" as const, content: `${FX}/ring-sweep.mp4` },
  flythrough: { kind: "video" as const, content: `${FX}/arena-flythrough.mp4` },
  ring: { kind: "image" as const, content: `${FX}/ring-night.jpg` },
  logo: { kind: "video" as const, content: `${FX}/logo-slam.mp4` },
  faces: { kind: "image" as const, content: `${FX}/faces-persona.jpg` },
  heels: { kind: "image" as const, content: `${FX}/heels-persona.jpg` },
  belt: { kind: "image" as const, content: `${FX}/belt.jpg` },
  thanks: { kind: "image" as const, content: `${FX}/outro-thanks.jpg` },
  sparks: { kind: "video" as const, content: `${FX}/gold-sparks.mp4` },
  smoke: { kind: "video" as const, content: `${FX}/purple-smoke.mp4` },
  confetti: { kind: "video" as const, content: `${FX}/confetti-loop.mp4` },
};

type Dress = "faces" | "heels" | "award" | "venue" | "house" | "stage";
...
const VISUAL_SCENES: VisualSpec[] = [
  ...
  { title: "Vote", durationSec: 20, isOverlay: true, center: ASSETS.ring, dress: "venue" },
  { title: "Score", durationSec: 20, isOverlay: true, center: ASSETS.ring, dress: "venue" },
  {
  ...
  { title: "Faces Arena", durationSec: 60, center: ASSETS.faces, dress: "faces" },
  { title: "Heels Arena", durationSec: 60, center: ASSETS.heels, dress: "heels" },
  { title: "Luxor Stage", durationSec: 60, center: ASSETS.flythrough, dress: "stage" },
  { title: "Award Ceremony", durationSec: 90, center: ASSETS.belt, dress: "award" },
...
function sceneKey(n: number) {
  return `wl-luxor:scene:${String(n).padStart(2, "0")}`;
}

function effectKey(n: number, panel: string) {
  return `wl-luxor:effect:scene${String(n).padStart(2, "0")}:${panel}`;
}
...
function wings(dress: Dress): { left: Asset; right: Asset } {
  switch (dress) {
    ...
    case "stage":
      return { left: ASSETS.ring, right: ASSETS.ring };
    default:
 ...
 */
export async function seedWrestleLocoLuxor(
  ctx: MutationCtx,
  ownerId: Id<"users">,
): Promise<Id<"shows">> {
  const result = await seedWrestleLocoLuxorDetailed(ctx, ownerId);
  return result.showId;
}

export async function seedWrestleLocoLuxorDetailed(
  ctx: MutationCtx,
  ownerId: Id<"users">,
) {
  ...
  if (!show) {
    const tagged = await ctx.db
      .query("shows")
      .withIndex("by_tag", (q) => q.eq("tag", "wrestleloco"))
      .collect();
 ...
 * wrestleloco 33-round template into the two-team grid.
 */
async function ensureLuxorPerformance(
  ...
  const existing = performances.find(
    (p) => p.title === PERFORMANCE_TITLE && p.tag === "wrestleloco",
  );
  if (existing) {
  ...
  const loco = requireLoco("wrestleloco");
...
export async function runWrestleLocoLuxorSeed(ctx: MutationCtx) {
  const owner = await resolveOwner(ctx);
  console.log(
    `seedWrestleLocoLuxor owner: ${owner.handle} (${owner.tier}) ${owner._id}`,
  );
  const show = await seedWrestleLocoLuxorDetailed(ctx, owner._id);
...
export const seedWrestleLocoLuxorRun = mutation({
...
export const inspect = query({
  ...
  handler: async (ctx) => {
    ...
    if (!show) {
      return { found: false as const, wrestlelocoShows: shows.map((s) => s.title) };
    }
    const scenes = await ctx.db
      .query("scenes")
      .withIndex("by_show", (q) => q.eq("showId", show._id))
